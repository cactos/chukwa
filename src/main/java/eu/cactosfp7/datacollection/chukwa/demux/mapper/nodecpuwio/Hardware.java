...
import static eu.cactosfp7.datacollection.chukwa.demux.mapper.nodecpuwio.HardwareConstants.HARDWARE_COLUMN_NAMES;
import static eu.cactosfp7.datacollection.chukwa.demux.mapper.nodecpuwio.HardwareConstants.HARDWARE_HEADER_SPLIT_COLUMNS;
import static eu.cactosfp7.datacollection.chukwa.demux.mapper.nodecpuwio.HardwareConstants.hardwareReadSpec;
...
//			String[] rowValues = recordEntry.trim().split("[\\r\\n]+");
			...
			while ((nextLine = reader.readNext()) != null){
				if(!nextLine[0].equals("DATE") && nextLine.length==3){
					log.info("wio:"+filename+" "+nextLine[0]+" "+nextLine[1]+" "+nextLine[2]);
...
//				log.info("wio:"+CellValues[0]+CellValues[1]+CellValues[2]);
...
//				}
//			}
			log.info("mean cpu"+mean_cpu+"totalcores"+totalCores);
			if(totalCores!=0)
			{
			AbstractProcessorConstants.addSingleValue("cpu_cores", totalCores, hardwareElements);
			AbstractProcessorConstants.addSingleValue("cpu_wio", mean_cpu/totalCores, hardwareElements);

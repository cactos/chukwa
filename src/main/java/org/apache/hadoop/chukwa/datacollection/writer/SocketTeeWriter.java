...
/**
 * Effectively a "Tee" in the writer pipeline. Accepts incoming connections on
 * port specified by chukwaCollector.tee.port. Defaults to 9094
 *
 * Protocol is as follows: Client ---> TeeWriter "RAW | WRITABLE <filter>" as
 * per DumpChunks.
 *
 * TeeWriter ---> Client "OK\n" In RAW mode TeeWriter ---> Client (length(int)
 * byte[length])* An indefinite sequence of length, followed by byte array.
 *
 * In Writable mode TeeWriter ---> Client (Chunk serialized as Writable)* An
 * indefinite sequence of serialized chunks
 *
 * In English: clients should connect and say either "RAW " or "WRITABLE "
 * followed by a filter. (Note that the keyword is followed by exactly one
 * space.) They'll then receive either a sequence of byte arrays or of
 * writable-serialized.
 *
 * Option chukwaCollector.tee.keepalive controls using TCP keepalive. Defaults
 * to true.
 *
 */
...
//  private final ExecutorService pool;
    ...
    class Tee implements Runnable {
        ...
        Filter rules;
        DataFormat fmt;
        final BlockingQueue<Tuple> sendQ;

        public Tee(Socket s) throws IOException {
            sock = s;
            //now initialize asynchronously
            sendQ = new ArrayBlockingQueue<Tuple>(QUEUE_LENGTH);
...
//                        c.write(out);
                        ...
                        ObjectOutputStream data = new ObjectOutputStream(bout);
                        data.writeObject(t.getValues());
                        data.flush();
//                        byte[] header = (c.getDataType() + "\n").getBytes();
...
//                        out.write(header);
    ...
    private class Tuple implements Serializable {
        ...
        public Tuple(String key, Map<String, String> values, Chunk chunk) {
            this.key = key;
            this.values = values;
            this.chunk = chunk;
        }
    ...
    List<Tee> tees;
    ChukwaWriter next;
    private final ChukwaConfiguration conf = new ChukwaConfiguration();
    ...
    MockTeeOutputCollector output;

    public SocketTeeWriter() {
        log.warn("SocketTeeWriter empty constructor called");
    }
...
//        StringBuilder keyBuilder = new StringBuilder();
        ...
        // Map the chunks!
        List<Tuple> mappedChunks = new LinkedList<Tuple>();
        try {
            for (Chunk chunk : chunks) {
                values = new HashMap<String, String>();
                output = new MockTeeOutputCollector();
                ...
                processor.process(new ChukwaArchiveKey(), chunk, output, Reporter.NULL);
                List<ChukwaRecordKey> keys = output.getChukwaRecordKeys();
                List<ChukwaRecord> records = output.getChukwaRecords();
                String tupleKey = null;
                for (int i = 0; i < keys.size(); i++) {
                    String[] keyParts = keys.get(i).getKey().split("/");
                    if (keyParts[0].equals("CNSnapshot") || keyParts[0].equals("VMSnapshot")) {
                        for (String field : records.get(i).getFields()) {
                            values.put(field, records.get(i).getValue(field));
                        }
                        if (values.get("dataType") == null) {
                            values.put("dataType", chunk.getDataType());
                        }
                        if (tupleKey == null) {
                            tupleKey = keyParts[1];
                        }
                ...
                if (tupleKey != null) {
                    Tuple tuple = new Tuple(tupleKey, values, chunk);
                    mappedChunks.add(tuple);
                }
            }
        } catch (Exception e) {
            log.error(ExceptionUtil.getStackTrace(e));
            throw new WriterException("Failed to process chunk data.");
        }
